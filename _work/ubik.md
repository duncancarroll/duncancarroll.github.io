...
We know that brainwaves correlate to state of consciousness, with sleep being at the low end (delta) and alertness being at the high end (gamma), and [thalamo-cortical resonance](https://en.wikipedia.org/wiki/Recurrent_thalamo-cortical_resonance) has been proposed as an explanation for how the brain integrates information into a coherent whole.  If we imagine that in the normal waking state consciousness is phase-synchronized to the brain, the vibrational state could be explained as the interference pattern produced when the two separate.

Granted this is highly speculative territory, but don't worry we're going to test it.
...
Now that we've taken numerous leaps of faith, let's take one more and imagine that the linkage between these two systems is purely electrical.  While there's no real reason to assume this (maybe it's magnetic?  Or chemical?  Or if you're Stuart Hameroff, scale-independent quantum oscillatory), electrical sensors are cheap so let's start there.

# Testing
While numerous [tests](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3918960/) have been conducted on subjects in an OBE state, they are usually focused on body and brain activity, but in our model we would only expect a modest difference, after all the "car" is still idling, it's just that the driver has left the vehicle.

Instead we will take a different approach and measure the electrical potential of a nearby "surrogate brain" (in this case a flask of saltwater), while a subject in an OBE state attempts to manipulate it and pass a message via Morse code.

Why saltwater?  It's conductive and provides an environment remotely similar to the brain.  One thing we don't know  is whether our "consciousness signal" needs an existing electrical signal to bind to or if it's strong enough to  just push electrons around by itself, so we will need two solutions:
...
# Contact
If you are interested in this study and promise not to be snarky feel free to drop me a line.